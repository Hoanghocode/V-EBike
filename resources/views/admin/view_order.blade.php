...
<div class="table-agile-info">
  ...
  <div class="panel panel-default">
   ...
   @php
     $i++;
     $subtotal = (float)$details->product_price*(int)$details->product_sales_quantity;
     $total+=$subtotal;
   @endphp
...
<td>    <img src="{{asset('/public/upload/product/'.$details->product->product_image) }}" height="100" width="100">
</td>    
    ...
    <td> 
      ...
      <input type="hidden"  value="{{$details->order_code}}" name="order_code_storage" class="order_code_storage_{{$details->product->product_quantity}}">
 ...
 @endif
    ...
    <td>{{number_format((float)$details->product_price,0,',',".")}} VNĐ</td>
    <td>{{number_format((float)$details->product->product_cost,0,',',".")}} VNĐ</td>     {{-- kết nối với product --}}
...
@csrf
  <select class="form-control order_details">
    <option value="1" id="{{$or->order_id}}" selected >chưa xử lý</option>
    <option value="2" id="{{$or->order_id}}">đã xử lý - đang giao hàng</option>
  </select>
</form>
@else 
<form> 
  @csrf
  <select class="form-control order_details">
    <option disabled value="1" id="{{$or->order_id}}" >chưa xử lý</option>
    <option value="2" id="{{$or->order_id}}" selected  >đã xử lý - đang giao hàng</option>
  </select>
