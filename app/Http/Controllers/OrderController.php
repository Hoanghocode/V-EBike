...
//lấy mã giảm giá với coupon_code
 ...
 // thêm
    ...
    $output = '
        ...
        <h1><center>Tiệm ăn vặt hoa nghĩa</center></h1>
    ...
    foreach($order_details_product as $product){
		$subtotal = 0; // Initialize $subtotal before the if condition

if (is_numeric($product['product_price']) && is_numeric($product['product_quantity'])) {
...
$output .= '
    <tr>
        ...
        <td>'.number_format(floatval($product->product_feeship), 0, ',', '.').'đ'.'</td>
     
        <td>'.number_format(floatval($product->product_price), 0, ',', '.').'đ'.'</td>
...
<tbody>
    <!-- Add content for the Ký tên section here -->
</tbody>
</table>';
