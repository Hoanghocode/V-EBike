...
class CheckoutController extends Controller
{
    ...
    // Check if 'book_number' key exists in the $data array
    if (array_key_exists('book_number', $data)) {
        // Proceed with the booking process
        $booking = new Book();
        $booking->book_name = $data['book_name'];
        $booking->book_email = $data['book_email'];
        $booking->book_number = $data['book_number'];
        $booking->book_phone = $data['book_phone'];
        $booking->book_time = $data['book_time'];
        $booking->book_notes = $data['book_notes'];
        $booking->short_select = $data['short_select'];
        $booking->payment_select = $data['payment_select'];
        ...
        $title_mail = "Thông báo Đặt Bàn thành công vào $now";
        ...
        $booking_array = array(
            ...
            'book_email' => $data['book_email'],
            'book_number' => $data['book_number'],
            'book_time' => $data['book_time'],
            'book_phone' => $data['book_phone'],
            'book_notes' => $data['book_notes'],
            'short_select' => $data['short_select'],
            'payment_select' => $data['payment_select'],
        );
        
        Mail::send('pages.mail.book_order', ['booking_array' => $booking_array], function ($message) use ($title_mail, $data) {
        ...
        // Handle the case when 'book_number' key is not present in the $data array
        // You can log an error, return a message, or perform other relevant actions
    }
}
...
//seo
 ...
 $category = Db::table('tbl_category_product')->where('category_status','0')->orderBy('category_id', 'desc')->get();
  $meta_desc = "Đặt bàn ngay để thưởng thức những món ăn ngon của chúng tôi"; 
  $meta_keywords = "Đặt bàn ngay để thưởng thức những món ăn ngon của chúng tôi";
  $meta_title = "Đặt bàn ngay để thưởng thức những món ăn ngon của chúng tôi";
  $url_canonical = $request->url();
    return view('pages.book.book',compact('brand_product','meta_keywords','meta_desc','url_canonical','meta_title','category','category_post','city'));
  }
}